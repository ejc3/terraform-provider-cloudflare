...
### Worker-native Access

A Worker destination protects one Worker across its routes, Custom Domains, `workers.dev` hostname, and preview deployments. Worker-native applications intentionally omit `domain`.
...
- `domain` (String) The primary hostname and path secured by Access. This domain will be displayed if the app is visible in the App Launcher. Omit this field for self-hosted applications that use a Worker destination.
...
- `worker_id` (String) The ID of the Cloudflare Worker to protect. Required for `worker` and `preview_worker` destinations, and invalid for account-wide Worker destinations.
